...
import sys,struct,socket,re,optparse,ConfigParser,os, datetime
...
print 'Lease time set:', datetime.timedelta(0, Lease*60)
...
class DHCPACK(Packet):
    fields = OrderedDict([
        ...
        ("Op51",              "\x33"),
        ("Op51Len",           "\x04"),
        ("Op51Str",           "\x00\x01\x51\x80"), #Lease time
        ("Op1",               "\x01"),
        ("Op1Len",            "\x04"),
        ("Op1Str",            ""),                  #Netmask
        ("Op15",              "\x0f"),
    ...
    def calculate(self):
        self.fields["Op51Str"] = struct.pack('!L',Lease*60)
